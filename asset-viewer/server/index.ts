...
const uploadTargetByExtension: Record<string, string> = {
  '.mp4': 'videos',
  '.png': 'images',
  '.jpg': 'images',
  '.jpeg': 'images',
...
async function readPrompts(): Promise<Record<string, PromptEntry>> {
  await ensurePromptsFile();
  try {
    const raw = await fs.readFile(PROMPTS_FILE, 'utf8');
    return raw.trim().length > 0 ? (JSON.parse(raw) as Record<string, PromptEntry>) : {};
  } catch {
    return {};
  }
}

async function writePrompts(data: Record<string, PromptEntry>): Promise<void> {
  await ensurePromptsFile();
  await fs.writeFile(PROMPTS_FILE, `${JSON.stringify(data, null, 2)}\n`, 'utf8');
}

async function scanFolder(folder: string, allowedExts: string[]): Promise<string[]> {
...
app.put('/api/prompts', async (req, res) => {
  try {
    ...
    if (body && typeof body === 'object' && 'path' in (body as Record<string, unknown>) && 'entry' in (body as Record<string, unknown>)) {
      const incoming = body as { path: string; entry: PromptEntry };
    ...
    await writePrompts(body as Record<string, PromptEntry>);
...
app.get('/api/asset-catalog', async (req, res) => {
  try {
    ...
    for (const line of lines) {
      if (line.startsWith('#')) { currentHeader = line; continue; }
...
app.get('/api/asset/*', async (req, res) => {
  const encoded = (req.params as Record<string, string>)[0] ?? '';
